...
#    3. Optionally override ANTD_BIN / ANTD_LOG_PATH there too.
...
# Load the operator's private config (wallet key, etc). Kept out of the repo.
$localConfig = Join-Path $PSScriptRoot "start-antd.local.ps1"
if (-not (Test-Path $localConfig)) {
    Write-Host "❌ Missing $localConfig" -ForegroundColor Red
...
if (-not $env:AUTONOMI_WALLET_KEY) {
    Write-Host "❌ AUTONOMI_WALLET_KEY is not set in start-antd.local.ps1" -ForegroundColor Red
    exit 1
}
...
# Quiet down saorsa spam so real errors stand out.
if (-not $env:RUST_LOG) {
    $env:RUST_LOG = "info,saorsa_core=warn,saorsa_transport=warn"
}

# Default binary path — override in start-antd.local.ps1 if yours is elsewhere.
if (-not $env:ANTD_BIN) {
    $env:ANTD_BIN = "C:\Users\USER\Desktop\Autonomi\ant-sdk\antd\target\release\antd.exe"
}

if (-not (Test-Path $env:ANTD_BIN)) {
    Write-Host "❌ antd binary not found at $env:ANTD_BIN" -ForegroundColor Red
